...
void Stack_Init(Stack *stack)
{
	...
	if (stack == NULL) // V případě, že funkce dostane jako parametr stack == NULL
		Stack_Error(SERR_INIT);
	stack->topIndex = -1;									  // Nastaveni indexu na -1 aby topIndex obsahoval pocet znaku
	stack->array = (char *)malloc(STACK_SIZE * sizeof(char)); // alokace pameti pro zasobnik
}
...
{
	return (stack->topIndex == -1); // vysledkem je logicka hodnota
}
...
{
	return (stack->topIndex == STACK_SIZE - 1); // vysledkem je logicka hodnota
}

/**
 * Vrací znak z vrcholu zásobníku prostřednictvím parametru dataPtr.
 * Tato operace ale prvek z vrcholu zásobníku neodstraňuje.
 * Volání operace Top při prázdném zásobníku je nekorektní a ošetřete ho voláním d
...
{
	bool stack_IsEmpty = Stack_IsEmpty(stack);
	if (stack_IsEmpty) // kontrola prazdnosti zasobniku
	{
		Stack_Error(SERR_TOP);
	}
	else
	{
		*dataPtr = stack->array[stack->topIndex]; // vraceni prvku z vrcholu
	}
}
...
{
	if (stack->topIndex >= 0) // podminka pro snizeni abychom se nedostali za -1
		stack->topIndex--;
}
...
{
	bool stack_IsFull = Stack_IsFull(stack);
	if (!stack_IsFull)
	{
		stack->array[stack->topIndex + 1] = data; // zvyseni o jedna, abychom mirili na prvni prvek v poli
		stack->topIndex++;						  // zvyseni indexu, zaroven ukazuje pocet prvku
	}
	else
	{
...
{
	free(stack->array);	  // uvolneni pameti
	stack->array = NULL;  // nastaveni na null kvuli detekci inizializace
	stack->topIndex = -1; // nastaveni na prazdou hodnotu
}
